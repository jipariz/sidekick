 ...
 *
 * Renders the full-screen debug panel with plugin list / plugin detail navigation. The **client**
 * is responsible for visibility (e.g. FAB + `AnimatedVisibility`); this composable only renders the
 * menu content itself.
 *
 * The plugins list is stabilized internally by its plugin ids, so a `listOf(...)` literal at the
 ...
 * - **`useSidekickTheme = false`** → the host app's ambient
 *   [androidx.compose.material3.MaterialTheme] is inherited as-is.
 *
 * @param plugins Plugins to show in the debug panel.
 * @param appInfo Optional host-app metadata shown in the panel header.
 * @param useSidekickTheme When true, apply the library's color scheme; when false, inherit the
 *   host's ambient MaterialTheme.
 * @param title Title shown in the plugin-list app bar.
 * @param navigationIcon Optional leading slot on the plugin-list app bar.
 * @param actions Optional trailing slot on the plugin-list app bar — a host typically wires a close
 *   button here.
 */
@Composable
fun Sidekick(
