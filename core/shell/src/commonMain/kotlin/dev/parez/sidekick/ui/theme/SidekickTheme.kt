 ...
 *
 * When [useSidekickTheme] is `true` (default), applies the library's own [SidekickLightColorScheme]
 * / [SidekickDarkColorScheme] based on the system dark-mode setting. Host typography and shapes are
 * preserved so a custom font / corner-radius configuration carries through into the overlay.
 *
 * When `false`, the host application's ambient [MaterialTheme] is inherited as-is.
 */
@Composable
internal fun SidekickTheme(useSidekickTheme: Boolean = true, content: @Composable () -> Unit) {
    when {
        useSidekickTheme -> {
            val colorScheme =
                if (isSystemInDarkTheme()) SidekickDarkColorScheme else SidekickLightColorScheme
            MaterialTheme(
