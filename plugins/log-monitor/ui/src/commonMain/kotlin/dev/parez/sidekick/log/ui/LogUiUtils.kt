...
internal fun LogLevel.label(): String =
    when (this) {
        LogLevel.VERBOSE -> "V"
        LogLevel.DEBUG -> "D"
        LogLevel.INFO -> "I"
...
internal fun LogLevel.fullLabel(): String =
    when (this) {
...
@Composable
internal fun LogLevel.onColor(): Color =
    when (this) {
...
/**
 * Tonal-container backgrounds for the pane-identity disc in the detail header. Mirrors [color] but
 * uses the M3 `*Container` roles so the disc reads as a recessed badge instead of a high-emphasis
 * fill.
 */
@Composable
internal fun LogLevel.containerColor(): Color =
    when (this) {
...
@Composable
internal fun LogLevel.onContainerColor(): Color =
    when (this) {
...
private fun pad3(n: Long): String =
    when {
        n < 10 -> "00$n"
        n < 100 -> "0$n"
        else -> "$n"
