...
class SidekickPreferencesProcessor(
    ...
    /**
     * Primitive types the KSP code generator can map to a `PreferenceDefinition`. Enums are handled
     * separately.
     */
    private val supportedPrimitiveTypes =
        setOf("Boolean", "String", "Int", "Long", "Float", "Double")
    ...
    private fun processClass(classDecl: KSClassDeclaration) {
        val annotation =
            classDecl.annotations.first { it.shortName.asString() == "SidekickPreferences" }
        val title =
            (annotation.arguments.firstOrNull { it.name?.asString() == "title" }?.value as? String)
                ?.takeIf { it.isNotEmpty() }
                ?: classDecl.simpleName.asString().replace(Regex("([A-Z])"), " $1").trim()

        val storeName =
            (annotation.arguments.firstOrNull { it.name?.asString() == "storeName" }?.value
                    as? String)
                ?.takeIf { it.isNotEmpty() }
        ...
        candidateProps.forEach { prop ->
            ...
            val isEnum =
                typeDecl is KSClassDeclaration && typeDecl.classKind == ClassKind.ENUM_CLASS
        ...
        logger.info(
            "Generating Sidekick classes for $className (title=$title, storeName=${storeName ?: "<derived>"})"
        )

        AccessorGenerator(codeGenerator, logger)
            .generate(packageName, className, title, properties, storeName)
    ...
    private fun buildPreferenceProperty(prop: KSPropertyDeclaration): PreferenceProperty {
        val prefAnnotation =
            prop.annotations.firstOrNull { it.shortName.asString() == "Preference" }
        val annotationLabel =
            (prefAnnotation?.arguments?.firstOrNull { it.name?.asString() == "label" }?.value
                    as? String)
                ?.takeIf { it.isNotEmpty() }
        val annotationDescription =
            (prefAnnotation?.arguments?.firstOrNull { it.name?.asString() == "description" }?.value
                as? String) ?: ""
    ...
    /**
     * KSP doesn't expose property initializer expressions. Read the line of source the property
     * starts on (and a small forward window for short multi-line initializers), locate `var <name>`
     * / `val <name>`, and extract the substring after the first un-typed `=` that follows. The `var
     * <name>` anchor is critical: KSP's `location` for an annotated property points at the
     * annotation's line, so naively scanning for `=` would otherwise hit the `=` inside an
     * annotation argument first. Returns null when the initializer can't be located — callers fall
     * back to a type-zero default.
     */
    private fun readPropertyInitializerSource(
        ...
        val joined = (startIdx until windowEnd).joinToString(" ") { stripLineComment(lines[it]) }
    ...
    /**
     * Convert the raw initializer expression to the value-string the generators expect (e.g.
     * `"false"`, `"42"`, `DEFAULT`, the unquoted string contents). The raw text may include
     * trailing source we don't care about (rest-of-line after the literal, comments, next-property
     * declarations); `firstLiteralToken` peels off only the leading expression token before we
     * type-classify it.
     */
    private fun normalisedDefault(
        ...
        return when {
            isEnum -> {
                ...
                if (candidate.isNotEmpty() && (enumValues.isEmpty() || candidate in enumValues))
                    candidate
            ...
            typeName == "String" -> {
                // Strip surrounding quotes from a Kotlin string literal; leave non-literals to
                // type-zero.
            ...
            typeName == "Int" ->
                token.removeSuffix("L").removeSuffix("l").toIntOrNull()?.toString() ?: "0"
            typeName == "Long" ->
                token.removeSuffix("L").removeSuffix("l").toLongOrNull()?.toString() ?: "0"
            typeName == "Float" ->
                token.removeSuffix("f").removeSuffix("F").toFloatOrNull()?.toString() ?: "0"
            typeName == "Double" ->
                token.removeSuffix("f").removeSuffix("F").toDoubleOrNull()?.toString() ?: "0"
     ...
     * - A sequence of identifier / number characters (`[A-Za-z0-9._+\-]`), good enough for
     *   primitives (`42L`, `0.5f`, `-3`) and qualified identifiers (`ColorTheme.DEFAULT`).
     */
    ...
    private fun decodeStringLiteral(content: String): String =
    ...
    private fun typeZero(typeName: String, isEnum: Boolean, enumValues: List<String>): String =
        when {
    ...
    private fun humaniseName(name: String): String =
        name.replace(Regex("([A-Z])"), " $1").trim().replaceFirstChar { it.uppercaseChar() }
}
