package dev.parez.sidekick.preferences.ksp.generator
...
class PluginGenerator(private val codeGenerator: CodeGenerator, private val logger: KSPLogger) {
    ...
    fun generate(
        ...
        val accessorParam =
            com.squareup.kotlinpoet.ParameterSpec.builder("accessor", accessorType)
                .defaultValue("%T()", accessorType)
                .build()

        // definitions = listOf(...)
        val definitionsBlock =
            CodeBlock.builder()
                .apply {
                    add("listOf(\n")
                    indent()
                    for (prop in properties) {
                        if (prop.isEnum) {
                            val enumPrefClass =
                                ClassName("dev.parez.sidekick.preferences", "EnumPref")
                            val optionsList = prop.enumValues.joinToString(", ") { "\"$it\"" }
                            add(
                                "%T(%S, %S, %S, %S, listOf($optionsList)),\n",
                                enumPrefClass,
                                prop.name,
                                prop.label,
                                prop.description,
                            ...
                            val prefClass =
                                ClassName(
                                    "dev.parez.sidekick.preferences",
                                    prop.definitionClassName(),
                                )
                            add(
                                "%T(%S, %S, %S, ${prop.defaultLiteral()}),\n",
                                prefClass,
                                prop.name,
                                prop.label,
                                prop.description,
                ...
                .apply {
                    ...
                    for (prop in properties) {
                        ...
                        if (prop.isEnum) {
                            ...
                            add(
                                "%S -> accessor.%N(%T.valueOf(value as %T))\n",
                                prop.name,
                                setterName,
                                enumType,
        ...
        val classBuilder =
            TypeSpec.classBuilder(pluginName)
                ...
                .primaryConstructor(
                    FunSpec.constructorBuilder().addParameter(accessorParam).build()
                )
                .addProperty(
                    PropertySpec.builder("accessor", accessorType).initializer("accessor").build()
                )

        val fileSpec =
            FileSpec.builder(packageName, pluginName).addType(classBuilder.build()).build()

        codeGenerator
            .createNewFile(Dependencies(false), packageName, pluginName)
            .bufferedWriter()
            .use { fileSpec.writeTo(it) }
    }
}

private fun PreferenceProperty.definitionClassName(): String =
    when (type) {
        "Boolean" -> "BooleanPref"
        "Int" -> "IntPref"
        "Long" -> "LongPref"
...
private fun PreferenceProperty.defaultLiteral(): String =
    when (type) {
