...
Each Sidekick BOM is built against a fixed Kotlin and Compose Multiplatform stack. Consumers need to match the Kotlin and Compose Multiplatform minor versions (klib ABI is not stable across minors) and meet the `compileSdk` constraint.
