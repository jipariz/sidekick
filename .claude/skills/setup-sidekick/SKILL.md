...
`AnimatedVisibility` pair. The Sidekick demo (`demo/shared`) ships a copy-paste-ready
...
`useSidekickTheme = false`. The SDK exposes only `Sidekick()`; the demo's
...
- Do **not** import `SidekickShell` from any `dev.parez.sidekick:*` artifact —
  ...
  visibility. (The demo has a same-named sample composable, but it is not
