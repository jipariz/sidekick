...
@Composable
fun colorSchemeFor(theme: ColorTheme, dark: Boolean): ColorScheme =
    when (theme) {
        ColorTheme.DYNAMIC ->
            dynamicColorScheme(dark) ?: if (dark) defaultDark() else defaultLight()
...
/**
 * Returns the platform's dynamic color scheme, or null if unavailable. On Android 12+ this uses the
 * user's wallpaper-derived colors.
 */
@Composable expect fun dynamicColorScheme(dark: Boolean): ColorScheme?
